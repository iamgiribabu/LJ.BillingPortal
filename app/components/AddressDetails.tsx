...
const AddressDetails = () => {
  ...
  const addressAdded : IAddressState = useSelector((state: RootState) => state.address);
  ...
  useEffect(() => {
    ...
    .then((data) => {setListOfAddress(data[0])}
    ).catch((error) => {
  ...
  const handleAddressSelect = (addr: IAddressState) => {
    ...
    setAddress(addr); // Set the selected address to the state
